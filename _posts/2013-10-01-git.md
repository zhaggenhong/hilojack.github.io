...
- git checkout
	working copy from index(priority) or switch to commit
- git revert
	working copy from commit( not include working copy or index change)
...
恢复本地
	...
	git reset HEAD  <file> //(reove stage), 则working不变
	git reset --hard origin/master //	working 丢失
...
### switch & add
	...
	git checkout -b test origin/test //tracking branch(真的)
	git checkout -t,--track origin/test //tracking branch
